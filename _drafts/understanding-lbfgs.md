...
Numerical optimization is at the core of much of machine learning. Once you've defined your model and have a dataset ready, estimating the parameters of your model typically boils down to minimizing some [multivariate function] $f(x)$, where the input $x$ is in some high-dimensional space and corresponds to model parameters. If we solve the unconstrained function minimization

$$
  x^* = \arg\min_x f(x)
$$

then $x^*$ is the 'right' choice for model parameters according to how you've set your objective.[^global-min]

In this post, I'll focus on this problem and not any of it's specific application in machine learning or outside of it (but trust me, there are many). Specifically, we'll derive the [L-BFGS] algorithm, commonly used in settings where 'batch' optimization makes sense. For a lot of larger problems, online methods based around [stochastic gradient descent] have gained popularity, since they require much less time to converge. In a later post, I might cover some fo these techniques, including my personal favorite [AdaDelta]. Throughout the post, I'll assume you remember multivariable calculus; so if you don't recall what a [gradient] or [Hessian] is, it's a good time to bone up.

# Newton's Method

Most numerical optimization procedures are iterative algorithms which consider a sequence of 'guesses' $x_n$ which ultimately converge to $x^*$ the true global minimizer of $f$. Suppose, we have an estimate $x_n$ and we want our next estimate $x_{n+1}$ to lower the value of $f$ $$(\mbox{e.g.} f(x_{n+1}) < f(x_n))$$. Without loss of generality, we can write $$x_{n+1} = x_n + \Delta x$$.

Newton's method is centered around a quadratic approximation of $f$ for points near $x_n$. Assuming that $f$ is twice-differentiable, we can use a quadratic approximation of $f$ for points 'near' $x_n$ using a [Taylor expansion]:
...
where $\nabla f(x_0)$ and $\nabla^2 f(x_0)$ are the gradient and Hessian of $f$ at the point $x_n$. This approximation holds in the limit as $\|\| \Delta x \|\| \rightarrow 0$. This is a generalization of the single-dimensional Taylor polynomial expansion you might remember from Calculus.

In order to simplify much of the notation, we're going to think of our iterative algorithm of producing a sequence of such quadratic approximations $h_n$. Re-writting the above equation,
...
Note that if $\hessian_n$ is postive semi-definite then we know the $\Delta x$ that makes the gradient zero yields a global minimum. Solving for $\Delta x$:[^why-global]

$$
\Delta x = - \invhessian_n \grad_n
$$

This suggests $\invhessian_n \grad_n$ as a good direction to move $x_n$ towards. In practice, we set $$x_{n+1} = x_n - \alpha (\invhessian_n \grad_n)$$ for some value of $\alpha$ where $h_n(\cdot)$ is a valid approximation for $f$.


## Iterative Algorithm

The above suggests an iterative algorithm where we take a sequence of steps, setting $\Delta x$ as in the above.
...
\begin{align}
 ...
 & \qquad \qquad \alpha_{n+1} = \min_{\alpha \geq 0} f(x_{n} - \alpha d) \\
 & \qquad \qquad x_{n+1} \leftarrow x_{n} - \alpha_{n+1} d
\end{align}
$$
...
Note that all we need to do in order to use this algorithm is provide a way to compute the value, gradient, and inverse Hessian at a point. This procedure can be treated as a blackbox for anything which satisfies the following Java interface:
...
With quite a bit of tedious math, you can prove that for a [convex function], the above procedure will converge to a unique global minimizer $x^*$, regardless of the choice of $x_0$. There are many non-convex functions that arise in ML (almost all latent variable models or deep nets), but this procedure can be used with a little more care taken with respect to initialization and optimization parameters.

## Huge Hessians

The central issue with $\mathbf{NewtonRhapson}$ is that we need to be able to compute the inverse Hessian matrix.[^implicit-multiply] Note that for ML applications, the dimensionality of the input to $f$ typically corresponds to the parameters of the underlying model. It's not unusual to have hundreds of millions of parameters; in fact for vision applications, people have trained models with [billions of parameters]. For these reasons, computing $\hessian$ or its inverse is not practical since this would require storing the number of parameters squared. For many functions, the Hessian may not even be analytically computable, let along representable.

Because of these reasons, $\mathbf{NewtonRhapson}$ is rarely used in practice to optimize functions corresponding to large problems. In practice, the above procedure works when we don't use the exact inverse Hessian.  


# Quasi-Newton

The $\mathbf{NewtonRhapson}$ method above can be generalized to work with an approximation of the inverse Hessian. We can amend the algorithm above to take a $\mbox{QuasiUpdate}$ parameter which updates the inverse Hessian approximation using the change in inputs and gradients:
...
& \qquad \qquad \alpha_{n+1} = \min_{\alpha \geq 0} f(x_{n} - \alpha d) \\
& \qquad \qquad x_{n+1} \leftarrow x_{n} - \alpha_{n+1} d \\
& \qquad \qquad \invhessian_{n+1} \leftarrow \mbox{QuasiUpdate}(\invhessian_{n},f, x_n,x_{n+1})
\end{align}
$$

In terms of software, we can blackbox optimize an arbitrary differentiable function using $\mathbf{QuasiNewton}$ assuming we get a quasi-newton approximation update policy. In Java this might look like this,
...
Here we assume that the the Quasi-newton update is a function of the delta in $x$ as well as the gradient, plus the past estimate.
...
Using the gradient of $h_{n+1}(\cdot)$ and cancelling terms we get

$$
\hessian_{n}(x_{n} - x_{n-1}) = (\grad_{n} - \grad_{n-1}) \\
$$

This yields the so-called "secant conditions" which ensures that $\hessian_{n+1}$ behaves like the Hessian at least for the diference $$(x_{n} - x_{n-1})$$.

Assuming $$\hessian_{n}$$ is invertible, then multiplying both sides by $$\invhessian_{n}$$ yields
...
Recall that the a hessian represents the matrix of 2nd order partial derivatives: $\hessian_{i,j} = \partial f / \partial x_i \partial x_j$. The hessian is symmetric since the order of differentiation doesn't matter.
...
Since the only use for $\invhessian_n$ is via the product $\invhessian_n \grad_n$, we only need the above procedure to use the BFGS approximation in $\mathbf{QuasiNewton}.


### L-BFGS: BFGS on a memory budget

The BFGS quasi-newton approximation has the benefit of not requiring us to be able to analytically compute the Hessian of a function. However, we still must maintain a history of the $s_n$ and $y_n$ vectors for each iteration. Since one of the core-concerns of the $\mathbf{NewtonRhapson}$ algorithm were the memory requirements associated with maintaining an Hessian, the BFGS Quasi-Newton algorithm doesn't address that since our memory use can grow without bounds.

The L-BFGS algorithm, simply truncates the $\mathbf{BFGSMultiply}$ update to use the last $m$ input differences and gradient differences. This means, we only need to store $s_n, s_{n-1},\ldots, s_{n-m-1}$ and $y_n, y_{n-1},\ldots, y_{n-m-1}$ to compute the update. The one interesting modification is that the center product, instead of using $\invhessian_0$, we use a diagonal matrix based upon the



# Why can't I just use a black-box optimizer
